...
class LoginForm extends Component {
  ...
  renderButton = buttonText => {
    const { isLogin, email, password, loginUser, createUser } = this.props;
    if (this.props.loading) {
      return <BarIndicator color="rgba(231,76,60,1)" size={60} />;
    }
    ...
    return (
      <Button
        text={buttonText}
        onPress={() =>
          this.onButtonPress(isLogin, email, password, loginUser, createUser)
        }
      />
    );
  };
...
export default connect(
  mapStateToProps,
  { loginChange, toSignUp, createUser, loginUser }
)(LoginForm);
